...
class StreamProcessExtractor extends Thread {
    ...
    public StreamProcessExtractor(StringBuffer buffer, InputStream stream, DownloadProgressCallback callback, String title) {
        this.stream = stream;
        this.buffer = buffer;
    ...
    public void run() {
        try {
            ...
            int nextChar;
            while ((nextChar = in.read()) != -1 && !isInterrupted()) {
            ...
            Log.d("태그", "(StreamProcessExtractor.java) interrupted !!");
    ...
    private void processOutputLine(String line) {
        ...
        if (m.matches()) {
            ...
            callback.onProgressUpdate(progress, eta, title);
        }
    }
